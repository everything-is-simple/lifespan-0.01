...
from development_governance_legacy_backlog import (
    LEGACY_HARD_OVERSIZE_BACKLOG,
    LEGACY_TARGET_OVERSIZE_BACKLOG,
)
...
def run_check(
    ...
    *,
    ...
    legacy_hard_backlog: list[str] = []
    legacy_target_backlog: list[str] = []
    ...
    for path in _candidate_files(repo_root, paths):
        ...
        if line_count > hard_max_lines:
            if not strict_mode and rel in LEGACY_HARD_OVERSIZE_BACKLOG:
                legacy_hard_backlog.append(f"{rel} ({line_count} 行)")
            else:
                hard_failures.append(f"{rel} ({line_count} 行)")
        elif line_count > target_max_lines:
            entry = f"{rel} ({line_count} 行)"
            if not strict_mode and rel in LEGACY_TARGET_OVERSIZE_BACKLOG:
                legacy_target_backlog.append(entry)
            else:
    ...
    if hard_failures:
        lines.append(f"  - 超过 {hard_max_lines} 行硬上限：")
        lines.extend(f"    - {entry}" for entry in sorted(hard_failures))
    else:
        scope_label = "本次改动范围" if paths else "全仓已跟踪文件"
        lines.append(f"  - 通过：{scope_label}没有文件超过 {hard_max_lines} 行硬上限。")

    if legacy_hard_backlog:
        lines.append("  - 历史超长白名单：")
        lines.extend(f"    - {entry}" for entry in sorted(legacy_hard_backlog))
    if legacy_target_backlog:
        lines.append("  - 历史超过目标上限旧债：")
        lines.extend(f"    - {entry}" for entry in sorted(legacy_target_backlog))
    if soft_warnings:
        lines.append(f"  - 超过 {target_max_lines} 行目标上限但未超过硬上限：")
...
def main() -> int:
    ...
    if args.report_path:
        ...
        report_path.write_text("\n".join(["# file length governance report", "", "```text", output_text, "```", ""]), encoding="utf-8")
