"""`data_tushare_objective` ? request ??????? materialization helper?"""
...
class _TushareRequestPlan:
    source_api: str; cursor_type: str; cursor_value: str

@dataclass(frozen=True)
class _ObjectiveEventRow:
    event_nk: str; asset_type: str; code: str; source_api: str; objective_dimension: str
    effective_start_date: date; effective_end_date: date | None; status_value_code: str | None
    status_value_text: str | None; source_trade_date: date | None; source_ann_date: date | None
    payload_json: str | None; first_seen_run_id: str; last_seen_run_id: str
...
def _fetch_and_normalize_tushare_request(
    *,
    ...
    if request_plan.source_api == "suspend_d":
        ...
        records = _normalize_suspend_rows(
            rows,
            run_id=run_id,
            request_nk=stable_request_ref,
            scope_codes=scope_codes,
    ...
    if request_plan.source_api == "stock_st":
        ...
        records = _normalize_stock_st_rows(
            rows,
            run_id=run_id,
            request_nk=stable_request_ref,
            scope_codes=scope_codes,
