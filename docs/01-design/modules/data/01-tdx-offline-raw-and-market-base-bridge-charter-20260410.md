...
```mermaid
flowchart LR
    TDX[H:/tdx_offline_Data] -->|文件级跳过+行级upsert| RAW[raw_market]
    RAW --> BASE_B[market_base backward → malf/structure/filter/alpha]
    RAW --> BASE_N[market_base none → position/trade]
    RAW --> BASE_F[market_base forward → 研究展示]
```
