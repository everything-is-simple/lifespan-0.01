...
2. 当前是否允许进入 `47`，正式启动 `position` 卡组
3. 若允许，`47 -> 55` 的后续 acceptance 责任如何继续保持对 `100-105` 的冻结

## 设计目标

1. 在进入 `position` 前设置最后一道正式 acceptance gate。
2. 把“是否允许进入 `47`”升级为正式 conclusion，而不是聊天判断。
3. 明确 `46` 是 `47` 的唯一直接前置 acceptance 卡；`100-105` 仍由 `55` 统一解冻。

## 核心裁决

1. `47` 只能在 `46` 接受后恢复为当前待施工卡。
2. `46` 必须基于 `43 / 44 / 45` 的正式 evidence / record / conclusion 做系统级裁决。
3. 若 `46` 不接受，必须新增新的前置修复卡，不得直接跳入 `47`。
