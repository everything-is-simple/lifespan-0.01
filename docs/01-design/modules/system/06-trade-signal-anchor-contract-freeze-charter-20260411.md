...
1. 本卡已不再以 `46` 为准入门槛，而必须以后续 `55-pre-trade-upstream-data-grade-baseline-gate` 的接受结论为启动条件。
2. 本卡属于 `100-105 trade/system` 恢复卡组的第一张卡；若 `55` 未通过，不得提前恢复。
3. 本卡收口后，必须为 `102 / 103` 提供稳定 anchor 输入，而不是只完成命名或注释层冻结。
...
5. 本卡默认要求显式冻结 `signal_anchor_contract_version` 或等价版本字段，用来标识 anchor 解释口径。
...
4. 本卡不替代 `102` 的退出账本与 realized pnl 设计。
