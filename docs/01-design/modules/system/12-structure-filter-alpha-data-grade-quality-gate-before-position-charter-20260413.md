...
3. 把当前后半部施工顺序改写为：
   ...
   - `43 -> 46` 上游质量闸门与 acceptance
   - `47 -> 55` `position / portfolio_plan` upstream A 级硬化
   - `100 -> 105` 下游执行恢复卡组
4. 让 `100` 不再被理解成“当前天然下一步”，而是“只有 `55` 接受后才能恢复的后置卡组入口”。
...
1. 接受：
   `structure / filter / alpha` 已达到进入 pre-position 硬化链的 data-grade 质量门槛，可继续 `44 -> 46`，并维持 `100-105` 冻结
2. 部分接受：
   `structure / filter / alpha` 已完成大部分对齐，但仍需先补一张或多张前置修复卡，再继续 `44 -> 46`
3. 拒绝：
   当前禁止继续进入 `44 -> 46 / 47 -> 55`，必须先完成上游质量补齐

### 4. 与 100 的关系

- `100` 继续保留为 `alpha -> position -> trade` 的正式 signal anchor freeze 卡。
- 但自本宪章生效起，`100` 的施工前置条件新增一条：
  `43 -> 55` 必须先按顺序完成并由 `55` 明确接受；`43` 只负责冻结上游质量门槛与阻断项。
...
3. 后续任何“进入 `position`”的提议，都必须先引用 `43` 的正式裁决，并继续经过 `44 -> 46`
4. 后续任何“恢复 `100-105`”的提议，都必须先引用 `55` 的正式接受结论
