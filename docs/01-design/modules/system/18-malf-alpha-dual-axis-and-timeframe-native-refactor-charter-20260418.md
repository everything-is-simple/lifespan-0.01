...
本轮新增 `78-80 / 91-95` 卡组，直接替换已经删除的 `78-84` 旧 middle-ledger 窗口建仓卡组，冻结新的正式主链口径：
...
7. `0/1` 波段问题必须先被固化成统一只读审计基线，再决定是否改 `canonical_materialization` 或重建 `malf_day / malf_week / malf_month`；不允许让 downstream 各自私带过滤口径。
...
`93` 必须显式裁决：`filter` 是继续保留薄 `filter.duckdb`，还是进一步降为 `alpha` 消费前的薄 sidecar 物化层。

### 7. cutover 先于续推

`78-80 / 91-95` 完成前，不允许恢复 `100-105`。
旧 `78-84` 窗口卡组删除，不再保留双线施工口径；当前正式新卡组为 `79 -> 80 -> 91 -> 92 -> 93 -> 94 -> 95`。
...
3. `80`：先冻结 `0/1` 波段过滤边界。
4. `91`：把 `malf canonical` 改成 timeframe native source 并完成三库 full coverage。
5. `92`：把 `structure` 收敛成 `structure_day / structure_week / structure_month` 三个薄投影层并完成 replay。
6. `93`：把 `filter` 收敛成 objective gate + note sidecar，并裁决是否保留独立本地库。
7. `94`：把 `alpha` 重绑为正式终审层，并按五个 PAS 日线库完成 historical replay。
8. `95`：做 `malf -> alpha` 官方 truthfulness 与 cutover gate。
...
4. `80` 必须先冻结 `0/1` 波段过滤边界，明确 canonical truth 与 filtered consumption 的分层合同，并固定 `scripts/malf/run_malf_zero_one_wave_audit.py` 作为三类短 wave 的统一只读审计基线；后续若改 `switch_mode`、`bar_count`、stale guard 合同或决定重建 `malf_day / malf_week / malf_month`，都必须保留变更前/后的同口径审计对照。`filter` 的默认 hard block 清单会收窄到客观可交易性与正式宇宙 gate；独立落库与否留待 `93` 裁决。
...
flowchart LR
    ...
    C80 --> C91["91 malf timeframe native source"]
    C91 --> C92["92 structure 薄投影 replay"]
    C92 --> C93["93 filter 降格 replay"]
    C93 --> C94["94 alpha 终审重绑 replay"]
    C94 --> C95["95 malf alpha cutover gate"]
    C95 --> C100["100-105 trade/system 恢复"]
```
