...
3. `structure` 保留，但只绑定 `malf_day`，降为单库薄事实投影层，不再按 `D/W/M` 再拆三层。
...
1. `structure` 只保留 `malf_day` 的薄事实投影职责，不承担高周期拆层，也不提前代替 `alpha` 做裁决。
...
本轮重构后，`structure / filter / alpha` 的正式默认入口仍先绑定 `malf_day`。
`malf_week / malf_month` 先作为正式高周期语义账本存在，不在本轮强行把所有下游同步改成三库政治。
...
4. `81`：把 `structure` 收敛成 `malf_day` 的薄投影层并完成 replay。
...
3. `structure` 的默认上游将固定为 `malf_day`，不再追加 `week/month` 对应的第二、第三套投影层。
