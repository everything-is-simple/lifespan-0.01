...
为 `structure / filter / alpha` 冻结一套进入 `position` 之前必须满足的 data-grade 质量门槛，并据此裁决是否允许继续 `44 -> 46` 与后续 `47 -> 55`；`100 -> 105` 继续由 `55` 统一控制。
...
### 3. 禁止继续进入 pre-position / position 卡组
...
flowchart TD
    ...
    GATE -->|接受| NEXT["44 structure/filter hardening"]
