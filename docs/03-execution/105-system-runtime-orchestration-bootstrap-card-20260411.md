...
- 问题：
  `27` 已建立最小 readout / audit，但正式 orchestration 仍未落地。
- 目标结果：
  建立 `system` 最小 runtime / orchestration bootstrap。
- 为什么现在做：
  这必须排在 `28-104` 全部之后，尤其要等 canonical malf、trade progression 和真实数据 smoke 都先通过。
