...
5. 在当前 Python 环境补装 `tushare`，完成 `Tushare` 与 `Baostock` 的第一轮 bounded probe。
...
- 第一轮 probe 已经出现清晰信号：
  - `Tushare` 能提供 `list_status / delist_date / suspend_d`，更接近历史事件源。
  - `Baostock` 能提供 `tradestatus / isST` 日级快照，但 `query_all_stock(day)` 对 ETF/BJ 覆盖不完整，不能直接当作完整 universe 真值。
