...
- `filter` 是否还保留独立本地库，用户明确表示尚未定死，因此本次没有假装做出实现结论，只把该物理落库裁决明确延期到 `82`。

## 备注

- `78` 收口的是“范围冻结”，不是代码改造；因此当前推进到 `79` 代表可以开始路径契约实现，不代表 `malf/alpha` 已完成 cutover。
- `alpha` 五 PAS 日线分库是在 `78` 先冻结口径，真正路径和 replay 落地留给 `83`。

## 记录结构图

```mermaid
flowchart LR
    STEP["范围冻结文档回填"] --> DEV["filter 落库裁决延期到 82"]
    DEV --> NOTE["79 开始实现，83 做 alpha 五 PAS 落地"]
    NOTE --> CON["78 接受"]
```
