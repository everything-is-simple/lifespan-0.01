...
- 问题：
- 问题：
  即使 `43/44/45` 各自完成，也仍需要一张系统级 acceptance 卡来正式裁决：当前是否允许进入 `position`，以及 `100` 是否可以恢复为下一卡。
- 目标结果：
- 目标结果：
  形成进入 `position` 前的最终 upstream acceptance gate；只有 `46` 接受后，`100` 才恢复为当前待施工卡。
- 为什么现在做：
- 为什么现在做：
...
2. 判断当前是否允许进入 `position`。
3. 若允许，恢复 `100`；若不允许，明确新的前置修复卡。
