...
- 目标结果：先把主线整改问题正式登记为 `60-66` 卡组，冻结范围、前移当前施工位，并明确原分段建库 `78-84` 进入挂起等待恢复状态。
- 为什么现在做：如果不先改施工位与索引口径，后续原分段建库卡组 `78-84` 会继续在错误前提下被当成默认续推主线，导致整改问题无法被正式记录、审计和收口。
...
2. 回填 `README / AGENTS / Ω / 00 / A / B / C`，把当前待施工卡切到 `60`，并把原分段建库 `78-84` 标记为挂起等待整改收口后恢复。
3. 冻结整改批次与后续恢复顺序：`60-66 -> 78-84 -> 100-105`。

## 实现边界

- 本卡只负责整改批次登记、执行位迁移与索引口径收紧。
- 本卡不直接修改 `structure / filter / alpha / wave_life` 正式实现。
- 本卡不裁决 `78-84` 是否废弃，只裁决其当前不再作为 active 卡组。
