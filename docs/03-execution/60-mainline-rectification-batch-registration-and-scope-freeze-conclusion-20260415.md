...
- 接受：`60 -> 66` 被正式登记为 `59` 之后、`80 -> 86` 之前的主线整改卡组。
- 接受：`80 -> 86` 被正式保留为整改后的 official middle-ledger 恢复卡组，不再作为当前默认续推卡组。
...
- 拒绝：在未登记整改批次的前提下，直接把 `59` 当作 `80-86` 或 `100-105` 的默认放行依据。

## 原因

1. `59` 只裁决了 `2010` pilot 的 truthfulness gate，并没有把整改问题本身登记为正式主线卡组。
2. 如果不先修正执行索引，后续会继续把 `80-86` 误当成默认 active 卡组，导致整改问题失去正式审计入口。
3. `trade / system` 的恢复前提仍然是：
   - `60 -> 66` 整改收口
   - `80 -> 86` official middle-ledger 恢复与 cutover gate 通过

## 影响

1. 当前正式主线顺序冻结为 `60 -> 61 -> 62 -> 63 -> 64 -> 65 -> 66 -> 80 -> 81 -> 82 -> 83 -> 84 -> 85 -> 86 -> 100 -> 101 -> 102 -> 103 -> 104 -> 105`。
...
flowchart LR
    ...
    C60 --> C61["61-65 分项整改"]
    C61 --> C66["66 恢复闸门"]
    C66 --> C80["80-86 official middle-ledger 恢复"]
