...
- 问题：`raw/base` 已完成 `day/week/month` 分库，但当前 `78 -> 79 -> 80 -> 91 -> 92 -> 93 -> 94 -> 95` 口径若不额外冻结 `0/1` 波段过滤边界，仍会让 `malf`、`structure`、`filter` 与 `alpha` 在后续卡里私带不同消费语义。
...
- 为什么现在做：如果不先把这几条冻结清楚，`79 -> 80 -> 91 -> 92 -> 93 -> 94 -> 95` 就会一边做路径、一边改架构口径，最后每张卡都带半个设计决策。
...
- 次层：`78 -> 80 -> 91 -> 95` 执行治理与卡组顺序冻结
- 上游输入：`18` 设计/规格、`77` 六库收口结果与现行 `Ω` 路线图
- 下游放行：`79 -> 80 -> 91 -> 92 -> 93 -> 94 -> 95` 的单卡施工顺序，以及 `95` 后是否允许恢复 `100-105`
- 本卡职责：先把双主轴、三库/三薄层/day gate/五 PAS 的正式边界写死，再让后续卡各自只做单一施工事项

## 任务分解

1. 冻结双主轴口径，确认 `malf` 是市场语义真值层，`alpha` 是决策真值层。
2. 冻结 `malf` 必须全覆盖，`92-94` 的 `2010 ~ 当前` bounded replay 只适用于 downstream，不适用于 `malf` 本体。
...
6. 把 `79 -> 80 -> 91 -> 92 -> 93 -> 94 -> 95` 切成一张卡只做一件事的顺序，并同步 `100-105` 的恢复前置条件。

## 实现边界

- 范围内：`78 -> 80 -> 91 -> 92 -> 93 -> 94 -> 95` 的模块主次关系、卡组顺序、`malf` 全覆盖例外、`filter` 的明确处理方案，以及 `alpha` 五库但不三套 trigger 库的边界。
...
| 卡组顺序 | `79 -> 80 -> 91 -> 92 -> 93 -> 94 -> 95` 一张卡只做一件事，`95` 后才看 `100-105` | 边做路径边改 authority，或绕过 `95` 先推下游 |

## 实施清单

| 切片 | 实施内容 | 交付物 |
| --- | --- | --- |
| 切片 1 | 回读 `18` 设计/规格与 `78 -> 80 -> 91 -> 95 / 100-105` 当前卡面，定位旧 authority 残留 | 差异清单 |
...
| 切片 4 | 冻结 `79 -> 80 -> 91 -> 92 -> 93 -> 94 -> 95` 顺序与 `95 -> 100-105` 的放行关系 | 卡组顺序与 gate 说明 |
...
| --- | --- | --- | --- |
| 双主轴冻结 | `malf` 与 `alpha` 的主权边界明确且无重叠 | `structure/filter` 仍持有隐性终审权 | `79 -> 80 -> 91 -> 95` 无法单卡施工 |
| `malf` 例外写死 | `malf` 全覆盖与 downstream bounded replay 被分开定义 | 仍把两者混写成一条完成度 | `80/95` 验收失真 |
| `structure` 三薄层 | `structure_day / week / month` 被写成正式默认口径 | 仍残留 `malf_day` 单层表述 | `91/92` 上下文边界不稳 |
| `filter_day` 边界 | `filter_day` 明确只做 objective gate + note sidecar | 仍把 `filter` 写成待定层或终审层 | `92/93` 会反复回滚 |
| `alpha` 五 PAS | 五个 PAS 日线库与“不做 `5 × 3`”写清 | `alpha` 继续单库混写或膨胀成 `5 × 3` | `94` 无法稳定 cutover |
| 下游放行 | `100-105` 前置条件明确改成 `95` 接受 | 仍残留旧 gate 或旧编号 | `100-105` upstream authority 不可信 |

## 收口标准

1. `79 -> 80 -> 91 -> 92 -> 93 -> 94 -> 95` 的职责顺序冻结清楚：
   - `79` 只做 `malf` 三库路径/表族契约
   - `80` 只做 `0/1` 波段过滤边界
   - `91` 只做 `malf` timeframe native source + 全覆盖
   - `92` 只做 `structure` 三薄层
   - `93` 只做 `filter_day` 薄 gate
   - `94` 只做 `alpha` 五 PAS 日线终审重绑
   - `95` 只做 truthfulness / cutover gate
...
flowchart LR
    ...
    C79 --> C80["80 0/1 波段过滤边界"]
    C80 --> C91["91 malf native source + 全覆盖"]
    C91 --> C92["92 structure D/W/M 三薄层"]
    C92 --> C93["93 filter_day objective gate"]
    C93 --> C94["94 alpha 五 PAS 日线终审"]
