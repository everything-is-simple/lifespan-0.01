...
- 接受：
  接受 `78` 范围冻结结果：`79 -> 80 -> 91 -> 92 -> 93 -> 94 -> 95` 正式改写为“`80` 先冻结 `0/1` 波段过滤边界，`91` 再完成 `malf` 三库全覆盖，随后依次推进 `92` 的 `structure` 三薄层、`93` 的 `filter_day` 客观 gate、`94` 的 `alpha` 五 PAS 日线终审，并由 `95` 再决定 cutover”的新卡组。
- 拒绝：
  拒绝继续沿用旧口径：
...
1. `malf` 是公共市场语义真值层，不是 downstream cutover sidecar；它必须全覆盖，不能被 `92-94` 的 bounded replay 口径吞掉。
...
5. 把这几件事先冻结后，`79 -> 80 -> 91 -> 92 -> 93 -> 94 -> 95` 才能真正做到“一张卡只做一件事”。

## 影响

1. `79 -> 80 -> 91 -> 92 -> 93 -> 94 -> 95` 的施工顺序被重排为：
   - `79`：`malf` 三库路径/表族
   - `80`：`0/1` 波段过滤边界
   - `91`：`malf` native source 与全覆盖
   - `92`：`structure_day / week / month`
   - `93`：`filter_day`
   - `94`：`alpha` 五 PAS 日线库
   - `95`：truthfulness / cutover
2. `malf` 与 downstream 的 replay 口径被正式拆开：
   - `malf` 必须全覆盖
   - `structure / filter / alpha` 可先做 `2010-01-01` 至当前 official `market_base` 覆盖尾部的 bounded replay
3. 当前待施工位在 `78` 收口后继续推进到 `79`，但 `79 -> 80 -> 91 -> 92 -> 93 -> 94 -> 95` 的内部边界已按新讨论口径重写。
