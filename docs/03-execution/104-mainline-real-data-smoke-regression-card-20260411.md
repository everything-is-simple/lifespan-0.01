...
- 问题：
  在 `103` 完成前跑全主线真实数据 smoke 没有意义，因为 `trade` 还不会真实推进到 exit/pnl；同时如果不显式承接 `85` 之后的新官方 upstream，真实 smoke 也会误把旧 upstream 假设当成通过条件。
- 目标结果：
  用 1-2 只真实股票跑通 `alpha -> position -> trade -> system` 的 bounded 主线回归，并验证这条链已经承接 `85` 之后的新官方 upstream。
- 为什么现在做：
...
- 前提：承接 `85` 之后的新官方 upstream
- 本卡职责：做新框架下的第一次真实端到端验收，而不是单纯跑通一个旧链条 smoke

## 任务分解

1. 明确真实 smoke 的上游必须是 `85` 之后的官方链条：五 PAS 日线 `alpha`、正式 `position` 桥接层、正式 `trade` 结果与 progression 账本。
...
| 设计项 | 正式口径 | 不接受情形 |
| --- | --- | --- |
| upstream 口径 | smoke 必须承接 `85` 后的新官方 upstream | 继续拿旧 `55` 时代 upstream 假设做验收 |
...
| 切片 | 实施内容 | 交付物 |
| --- | --- | --- |
| 切片 1 | 明确 smoke 标的、窗口与 `85` 后 upstream 验收清单 | smoke checklist |
...
| --- | --- | --- | --- |
| 新 upstream 验收 | smoke 明确承接 `85` 后官方链条 | 仍沿用旧 upstream 假设 | `105` 输入失真 |
...
1. `104` 明确成为 `85` 之后新官方 upstream 的端到端验收卡。
