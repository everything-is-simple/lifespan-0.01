...
- 问题：`60-65` 已分别裁清主线整改的 scope、coverage、boundary、`wave_life` 真值与 `formal signal` admission authority，但系统仍缺少一张正式 gate 卡来判断这些整改是否已经足以恢复 `78-84`。
- 目标结果：正式冻结“`78-84` 是否恢复、按原顺序恢复还是继续补卡”的系统级裁决，并把该裁决写回执行索引、路线图与入口文件。
- 为什么现在做：`65` 已把 `alpha formal signal` 的 final admission authority 正式收回，`60-65` 的整改闭环已经具备被统一复核的前提；若不立即进入 `66`，仓库会再次陷入“到底能不能恢复 `78-84`”的口头状态。
...
2. 判定 `78-84` 是否可恢复、是否需要改写、还是必须继续追加整改卡。
