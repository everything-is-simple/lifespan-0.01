...
- 问题：当前 canonical `malf` 里，`break`、旧结构失效与新结构确认之间没有被正式分层，导致大量只触发旧结构失效的 bar 被错误沉淀成完成 wave。
- 目标结果：把 `break / invalidation / confirmation` 冻结成正式 truth contract，明确什么是触发、什么是失效、什么是确认，以及这些语义如何映射到 `wave / state / snapshot`。
...
1. 冻结 `break` 只表示旧结构门槛被击穿，不再允许它直接等价于新顺确认。
2. 冻结 `invalidation` 作为旧顺结构正式失效的账本解释，并明确它与 `牛逆 / 熊逆` 过渡态的关系。
3. 冻结 `confirmation` 只能由新的 `HH / LL` 推进成立，不允许由单个 `break` 替代。
4. 明确 `wave_ledger / state_snapshot / downstream thin projection` 只读消费哪些语义层，不得反向简化掉合同分层。
...
stateDiagram-v2
    ...
    Invalidated --> ConfirmedTrend: new HH / LL confirmed
...
flowchart LR
    ...
    T --> C["confirmation by new HH/LL"]
    C --> W["new confirmed wave lifecycle"]
```

## 收口标准

1. `break / invalidation / confirmation` 三层语义被正式写入 card / evidence / record / conclusion。
2. `break != confirmation` 被明确钉死为正式合同，不再允许用口头解释替代。
3. 过渡态与确认态的账本映射被正式写清。
4. `83-85` 可以直接以本卡结论为上游输入。
