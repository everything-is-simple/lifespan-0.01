...
- 次层：`100-105` 恢复放行入口
- 上游输入：`79-83` 的 official run summary、evidence、record 与对应结论页
...
| 放行语义 | `84` 接受才允许恢复 `100-105`；否则继续 upstream 补卡 | 口径未定时先恢复 `100-105` |
...
| 切片 | 实施内容 | 交付物 |
| --- | --- | --- |
| 切片 1 | 汇总 `79-83` official run summary、证据与记录 | 审计输入包 |
| 切片 2 | 分别审计 `malf` 全覆盖与 `structure/filter/alpha` bounded replay | 审计摘要 |
| 切片 3 | 核对 `filter_day` 五类 gate 与 `alpha` 五 PAS 主权边界 | 差异说明 |
| 切片 4 | 形成 `accept / reject / reopen upstream cards` 裁决 | `84` 结论 |
| 切片 5 | 回填 `84` evidence / record / indexes，并同步 `100-105` 放行状态 | execution 闭环 |
