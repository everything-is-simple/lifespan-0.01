...
- 拒绝：把“文档里已经写明 78-84 必须走 bounded full-window”视为足够，不再需要执行口径防呆。
...
2. `78-84` 年度历史建库必须走 bounded full-window
...
2. 后续 `78-84` 若误用无参 CLI，不会再静默生成 tail-sparse 历史库，而会直接失败暴露问题。
