...
- 接受：
  当真实 bounded smoke 明确承接 `84` 之后的新官方 upstream，并验证 `alpha -> position -> trade -> system` 四层都只使用正式账本时接受。
- 拒绝：
...
3. `84` 已经冻结 upstream 主权边界，`104` 必须验证下游是否真的承接了那条官方链。
...
```mermaid
flowchart LR
    U["84 后官方 upstream"] --> A["alpha -> position -> trade -> system smoke"]
