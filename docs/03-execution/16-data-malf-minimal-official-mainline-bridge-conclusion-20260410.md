...
> 口径补充：本结论冻结的是 `data -> malf -> structure` 的 bridge v1 主线补齐结果。
> `malf` 的纯语义核心已在 `23-malf-pure-semantic-ledger-boundary-freeze-conclusion-20260411.md` 中进一步收缩；当前 `card 16` 不再代表 `malf core` 的终局定义。
...
2. `H:\Lifespan-report\data\card16\malf-001.json` 与 `H:\Lifespan-report\data\card16\structure-001.json` 已证明现有 `structure` runner 能真实消费新生成的官方 `malf` 上游。
   - 此处“官方 `malf` 上游”指当时冻结的 bridge v1 兼容输出，而不是后来由 `23` 号结论收缩出的 pure semantic core。
